...
use miniscript::signer::{SignerId, SignersContainer};
...
impl PKOrF {
    fn from_key(k: &DescriptorKey) -> Self {
        match k {
            DescriptorKey::PubKey(pubkey) => PKOrF {
                pubkey: Some(*pubkey),
                ..Default::default()
            },
            DescriptorKey::XPub(xpub) => PKOrF {
...
fn signer_id(key: &DescriptorKey) -> SignerId<DescriptorKey> {
    match key {
        DescriptorKey::PubKey(pubkey) => pubkey.to_pubkeyhash().into(),
        DescriptorKey::XPub(xpub) => xpub.root_fingerprint().into(),
    }
}

fn signature(key: &DescriptorKey, signers: Arc<SignersContainer<DescriptorKey>>) -> Policy {
...
fn signature_key(
    key_hash: &<DescriptorKey as MiniscriptKey>::Hash,
    signers: Arc<SignersContainer<DescriptorKey>>,
) -> Policy {
...
impl<Ctx: ScriptContext> ExtractPolicy for Miniscript<DescriptorKey, Ctx> {
    fn extract_policy(
        &self,
        signers: Arc<SignersContainer<DescriptorKey>>,
...
impl ExtractPolicy for Descriptor<DescriptorKey> {
    fn extract_policy(
        &self,
        signers: Arc<SignersContainer<DescriptorKey>>,
    ) -> Result<Option<Policy>, Error> {
